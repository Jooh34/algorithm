...
def get_str_phoneme_len(str):
    len = 0
    for c in str:
        len += get_char_phoneme_len(c)
...
def get_SED(s1, s2):
    ...
    def SED(i, j):
        ...
        if i == 0:
            return get_str_phoneme_len(s2[:j]) * alpha

        if j == 0:
            return get_str_phoneme_len(s1[:i]) * alpha

        v1 = SED(i, j-1) + get_char_phoneme_len(s2[j-1]) * alpha
        v2 = SED(i-1, j) + get_char_phoneme_len(s1[i-1]) * alpha
